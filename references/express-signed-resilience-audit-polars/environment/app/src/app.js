...
/** Create the gateway resilience-audit application and load its startup policy. */
export function createApp({
  ...
  // TODO: AUDIT_MAX_CLOCK_SKEW_MS must be read and validated here, and the
  // route below must enforce timestamp freshness and nonce replay protection.
  const policy = loadPolicy(policyPath);
  const app = express();

  // The parser currently runs before signature authentication and discards the
  // exact wire bytes. The endpoint then signs a JSON reconstruction below and
  // never inspects the timestamp or nonce headers.
