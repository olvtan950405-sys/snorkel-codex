...
class Release:
    ...
    def __init__(self, root: Path, mirror_name: str = "mirrors"):
        ...
        self.keys.mkdir(parents=True); self.private.mkdir(); self.mirrors.mkdir()
        for key in ("alpha", "beta", "gamma"):
            run("openssl", "genpkey", "-algorithm", "RSA", "-pkeyopt", "rsa_keygen_bits:2048", "-out", str(self.private / f"{key}.pem"))
            run("openssl", "pkey", "-in", str(self.private / f"{key}.pem"), "-pubout", "-out", str(self.keys / f"{key}.pem"))
        self.models = [
    ...
    def _model(self, mirror: str, model: str, artifact: str, content: bytes) -> dict[str, object]:
        ...
        run("git", "init", "-q", str(repo)); run("git", "config", "user.name", "Fixture", cwd=repo)
        ...
        run("git", "add", ".", cwd=repo); run("git", "commit", "-qm", "model snapshot", cwd=repo)
        ...
        run("git", "remote", "add", "origin", str(remote), cwd=repo); run("git", "lfs", "push", "--all", "origin", cwd=repo)
        return {"mirror": mirror, "model": model, "artifact": artifact, "content": content,
                "digest": hashlib.sha256(content).hexdigest(), "size": len(content), "commit": commit, "remote": remote}
    ...
    def lock(self, name: str = "release.lock", *, quorum: int = 2,
        ...
        for index, model in enumerate(self.models):
            ...
            lines.append(f"model {model['model']} {model['mirror']} approved-v2 {model['commit']} {model['artifact']} {digest} {model['size']}")
        prefix = ("\n".join(lines) + "\n").encode()
...
def test_signature_binds_every_model_record(release: Release) -> None:
    ...
    path = release.lock(); raw = path.read_bytes()
    raw = raw.replace(str(release.models[0]["commit"]).encode(), b"0" * 40, 1); path.write_bytes(raw)
...
def test_lightweight_tag_does_not_satisfy_annotated_tag_binding(release: Release) -> None:
    ...
    model = release.models[0]; remote = Path(model["remote"])
...
def test_lfs_pointer_and_materialized_digest_are_both_enforced(release: Release) -> None:
    ...
    assert reasons == ["ARTIFACT_DIGEST_MISMATCH", "LFS_POINTER_INVALID"]
...
def test_each_rebase_marker_is_reported_as_conflict(release: Release, marker: str) -> None:
    ...
    path = release.root / "conflict.lock"; path.write_text(f"release-lock 1\n{marker}\n")
...
def test_manifest_grammar_rejects_unsafe_or_ambiguous_forms(release: Release, mutation: str) -> None:
    ...
    path = release.lock(); raw = path.read_bytes()
    if mutation == "traversal": raw = raw.replace(b"weights.bin", b"../weights.bin", 1)
    elif mutation == "duplicate": raw = raw.replace(b"signer beta ", b"signer alpha ", 1)
    elif mutation == "unsorted": raw = raw.replace(b"model org/encoder ", b"model zzz/encoder ", 1)
    elif mutation == "crlf": raw = raw.replace(b"\n", b"\r\n")
    else: raw = raw.replace(b"quorum 2", b"quorum 0", 1)
...
def test_required_tools_remain_active_in_lua_implementation() -> None:
    """The tool-specific solution invokes Git, LFS, OpenSSL, and sha256sum."""
    source = Path("/app/release_attestor.lua").read_text()
...
def test_shipped_release_is_repaired_without_mutating_trust_inputs() -> None:
    ...
    lock = Path("/app/release.lock"); before = hashlib.sha256(lock.read_bytes()).digest()
