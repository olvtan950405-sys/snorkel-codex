...
async function main() {
  ...
  let remotes:any=cfg.policyRemotes;
  try { if (process.env.POLICY_REMOTES) remotes=JSON.parse(process.env.POLICY_REMOTES); } catch { remotes=null; }
  const db=process.env.AUDIT_DB || cfg.auditDatabase, source=process.env.SOURCE_REPO || "/app";
  if (typeof cfg.policyRef!=="string" || !/^refs\/tags\/v\d+\.\d+\.\d+$/.test(cfg.policyRef)) { reasons.add("POLICY_REF_INVALID"); return reject(); }
  if (!Array.isArray(remotes) || remotes.length<2 || remotes.some((x:any)=>typeof x!=="string" || !x) || new Set(remotes).size!==remotes.length) { reasons.add("POLICY_QUORUM_FAILED"); return reject(); }
  if (!/^[0-9a-f]{40}$/.test(cfg.policyCommit) || !safePath(cfg.policySubmodule)) { reasons.add("POLICY_PIN_MISMATCH"); return reject(); }
  let resolved="";
  try {
    for (const remote of remotes) {
      const lines=(await git(["ls-remote","--tags","--",remote,cfg.policyRef,cfg.policyRef+"^{}"])).trim().split("\n").filter(Boolean);
      const peeled=lines.find(x=>x.endsWith("^{}")); const direct=lines.find(x=>!x.endsWith("^{}")); const found=(peeled||direct||"").split(/\s+/)[0]||"";
      if (found!==cfg.policyCommit) { reasons.add("POLICY_QUORUM_FAILED"); return reject(); }
      resolved=found;
    }
  ...
  } catch { reasons.add("POLICY_QUORUM_FAILED"); }
  if (reasons.size) return reject();
  try {
  ...
  try {
    ...
    const primary=join(temp,"mirror-0");
    await git(["clone","-q","--no-checkout","--",remotes[0],primary],{env}); await git(["-C",primary,"checkout","-q","--detach",resolved],{env});
    let policy:any;
    try { policy=JSON.parse(await git(["-C",primary,"show",resolved+":policy.json"])); } catch { reasons.add("POLICY_INVALID"); return reject(); }
    ...
    for (const c of exp.charts) {
      let pointer=""; try { pointer=await git(["-C",primary,"show",resolved+":"+c.thumbnail]); } catch {}
    ...
    for (let index=0; index<remotes.length; index++) {
      const checkout=index===0 ? primary : join(temp,"mirror-"+index);
      try {
        if (index>0) { await git(["clone","-q","--no-checkout","--",remotes[index],checkout],{env}); await git(["-C",checkout,"checkout","-q","--detach",resolved],{env}); }
        await git(["-C",checkout,"lfs","pull","origin"],{env});
      } catch { reasons.add("THUMBNAIL_MISMATCH"); continue; }
      for (const c of exp.charts) {
        try { const bytes=await readFile(join(checkout,...c.thumbnail.split("/"))); if (bytes.length!==c.size || createHash("sha256").update(bytes).digest("hex")!==c.oid) reasons.add("THUMBNAIL_MISMATCH"); }
...
commit=$(git --git-dir=/srv/policy-remotes/export-policy-a.git rev-parse refs/tags/v1.0.0^\{commit\})
