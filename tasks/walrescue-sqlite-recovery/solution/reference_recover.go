...
func decodePageSize(v uint32) (uint32, error) {
	...
	if v < 512 || v > 65536 || v&(v-1) != 0 {
		return 0, errors.New("unsupported page size")
	}
	return v, nil
}
...
func atomicWrite(path string, data []byte) error {
	...
	f, err := os.CreateTemp(dir, ".walrescue-*")
	if err != nil {
		return err
	}
	name := f.Name()
...
func rejectSymlink(path string) error {
	...
	if info.Mode()&os.ModeSymlink != 0 {
		return errors.New("output paths must not be symlinks")
	}
	return nil
}

func validatePaths(db, wal, out, report string) error {
	paths := []string{db, wal, out, report}
	names := make([]string, len(paths))
	for i, path := range paths {
		name, err := cleanAbs(path)
		if err != nil {
			return err
		}
		names[i] = name
	}
	labels := []string{"db", "wal", "out", "report"}
	for i := range names {
		for j := i + 1; j < len(names); j++ {
			if names[i] == names[j] {
				return errors.New(labels[i] + " and " + labels[j] + " paths must be distinct")
			}
		}
	}
	if err := rejectSymlink(out); err != nil {
		return err
	}
	return rejectSymlink(report)
}

func recoverBytes(base, wal []byte) ([]byte, Report, error) {
	var r Report
	if len(base) < 20 || string(base[:16]) != "SQLite format 3\x00" {
		return nil, r, errors.New("invalid base database")
	}
	bv := uint32(binary.BigEndian.Uint16(base[16:18]))
	bps, err := decodePageSize(bv)
	if err != nil || len(base)%int(bps) != 0 {
		return nil, r, errors.New("invalid base database page size")
	}
	if len(wal) < 32 {
		return nil, r, errors.New("truncated WAL header")
	}
	magic := binary.BigEndian.Uint32(wal[:4])
	little := magic == 0x377f0682
	if !little && magic != 0x377f0683 {
		return nil, r, errors.New("unsupported WAL magic")
	}
	if binary.BigEndian.Uint32(wal[4:8]) != 3007000 {
		return nil, r, errors.New("unsupported WAL version")
	}
	ps, err := decodePageSize(binary.BigEndian.Uint32(wal[8:12]))
	if err != nil || ps != bps {
		return nil, r, errors.New("page size mismatch")
	}
	...
	if s0 != binary.BigEndian.Uint32(wal[24:28]) || s1 != binary.BigEndian.Uint32(wal[28:32]) {
		return nil, r, errors.New("bad header checksum")
	}
	...
	for offset+slot <= len(wal) {
		scanned++
		h := wal[offset : offset+24]
		pg := binary.BigEndian.Uint32(h[:4])
		...
		page := wal[offset+24 : offset+slot]
		input := make([]byte, 8+len(page))
		copy(input, h[:8])
		copy(input[8:], page)
		...
		offset += slot
		if commit != 0 {
			size := int(commit) * int(ps)
			if len(work) < size {
				work = append(work, make([]byte, size-len(work))...)
			} else {
				work = work[:size]
			}
			for _, f := range tx {
				start := int(f.page-1) * int(ps)
				copy(work[start:start+int(ps)], f.data)
			}
			durable = append(durable[:0], work...)
			committed = valid
			transactions++
			dbPages = commit
			tx = tx[:0]
...
finished:
	if stop == "end_of_wal" && offset < len(wal) {
		stop = "partial_frame"
	}
	if transactions == 0 {
		return nil, r, errors.New("WAL contains no valid commit")
	}
	durable[18], durable[19] = 1, 1
	digest := sha256.Sum256(durable)
	r = Report{
		Status:            "recovered",
		PageSize:          ps,
...
func Run(args []string) error {
	...
	if *db == "" || *wal == "" || *out == "" || *rp == "" {
		return errors.New("all path flags are required")
	}
	if fs.NArg() != 0 {
		return errors.New("unexpected positional arguments")
	}
	if err := validatePaths(*db, *wal, *out, *rp); err != nil {
		return err
	}
	os.Remove(*out)
	os.Remove(*rp)
	...
	output, report, err := recoverBytes(base, wb)
	if err != nil {
		return err
	}
	if err = atomicWrite(*out, output); err != nil {
		return err
	}
	...
	if err = atomicWrite(*rp, append(j, '\n')); err != nil {
		os.Remove(*out)
		return err
	}
	return nil
