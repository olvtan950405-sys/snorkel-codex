...
class Fixture:
    def __init__(self, root: Path, remote_name='policy.git'):
        self.root=root; self.work=root/'policy'; self.remote=root/remote_name; self.source=root/'source'
        ...
        run('git','clone','-q','--bare',str(self.work),str(self.remote)); run('git','remote','add','origin',str(self.remote),cwd=self.work); run('git','lfs','push','--all','origin',cwd=self.work)
        e=os.environ.copy(); e['GIT_ALLOW_PROTOCOL']='file'; run('git','submodule','add','-q',str(self.remote),'vendor/policy',cwd=self.source,env=e)
    ...
    def write_config(self, **changes):
        value={'policyRemote':str(self.remote),'policyRef':'refs/tags/v2.3.4','policyCommit':self.commit,'policySubmodule':'vendor/policy','auditDatabase':str(self.db)}; value.update(changes); self.config.write_text(json.dumps(value))
...
def test_shipped_config_is_repaired():
    cfg=json.loads(Path('/app/config/worker.json').read_text()); assert cfg['policyRef'].startswith('refs/tags/v') and len(cfg['policyCommit'])==40
