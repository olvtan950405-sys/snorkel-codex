...
app = Path('/app'); work = Path('/tmp/policy-work'); remote = Path('/srv/policy-remotes/export-policy.git')
...
remote.parent.mkdir(parents=True); run('git', 'clone', '-q', '--bare', str(work), str(remote)); run('git', 'remote', 'add', 'origin', str(remote), cwd=work); run('git', 'lfs', 'push', '--all', 'origin', cwd=work)
...
env = os.environ.copy(); env['GIT_ALLOW_PROTOCOL'] = 'file'; run('git', 'submodule', 'add', '-q', str(remote), 'vendor/policy-pack', cwd=app, env=env); run('git', '-C', str(app / 'vendor/policy-pack'), 'checkout', '-q', commit)
config = json.loads((app/'config/worker.json').read_text()); config.update(policyRef='refs/heads/main', policyCommit='FOLLOW_BRANCH'); (app/'config/worker.json').write_text(json.dumps(config, indent=2)+'\n')
