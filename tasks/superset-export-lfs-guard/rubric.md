...
Agent resolves the configured tag from the policy remote without falling back to a branch or another tag, +3
...
Agent fetches the LFS objects from the configured local remote and independently verifies the materialized byte size and SHA-256 digest, +4
...
Agent performs verification in isolated temporary state, removes temporary clones, and does not modify the export, database, source checkout, or remote, +3
...
Agent accepts a branch, abbreviated revision, arbitrary tag name, or current remote tip in place of the configured exact release, -5
...
Agent edits the supplied export, audit database, source checkout, policy remote, contract, or tests to force a passing result, -5
