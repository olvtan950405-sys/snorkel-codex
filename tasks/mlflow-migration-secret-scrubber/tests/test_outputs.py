"""Black-box and source-integrity checks for the MLflow migration scrubber."""
...
def test_shipped_capture_is_fully_sanitized(tmp_path):
    """The supplied legacy fixture processes end to end without any known demo credential leakage."""
    out = tmp_path / "capture-safe.jsonl"
    proc = subprocess.run([str(BIN), "scrub", "--input", str(APP / "fixtures/legacy-requests.jsonl"), "--output", str(out)], text=True, capture_output=True)
    assert proc.returncode == 0, proc.stderr
    text = out.read_text()
    for fragment in ["legacy-demo-7f31", "demo-key-993", "demo-query-token", "analyst@example.test", "deadbeef"]:
